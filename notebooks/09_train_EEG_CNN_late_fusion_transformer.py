...
class LateFusionTransformer(nn.Module):
    def __init__(self, eeg_model, audio_model, d_model=128, nhead=4, num_layers=2, num_classes=2, dropout=0.1):
        super(LateFusionTransformer, self).__init__()
        # Encoders
        self.eeg_model = eeg_model
        self.audio_model = audio_model
        # removing the final classification layers
        self.eeg_model.fc = nn.Identity()
        self.audio_model.fc = nn.Identity()

        # Projections into a shared space
        self.eeg_proj = nn.Linear(32, d_model)
        self.audio_proj = nn.Linear(128, d_model)

        self.pos_encoder = nn.Parameter(torch.randn(2, d_model))
    ...
    def forward(self, eeg, audio):
        ...
        # Forming a sequence: (batch, seq_len=2, d_model)
        seq = torch.stack([eeg_emb, audio_emb], dim=1)

        # Adding positional encoding
        seq = seq + self.pos_encoder.unsqueeze(0)  # (batch, 2, d_model)
...
def create_model(model_type, num_classes=2, freeze=False):
    if model_type == 'eeg':
        model = EEGNet(num_classes=num_classes)
    elif model_type == 'audio':
        model = AudioCNN(num_classes=num_classes)
    elif model_type == 'late_fusion':
        eeg_model = EEGNet(num_classes=num_classes)
        audio_model = AudioCNN(num_classes=num_classes)
        model = LateFusionTransformer(eeg_model, audio_model, d_model=128, nhead=4, num_layers=2)
        if freeze:
...
def train_epoch(model, dataloader, criterion, optimizer, device):
    ...
    for eeg, audio, labels in dataloader:
        ...
        optimizer.zero_grad()
        if isinstance(model, (MultimodalModel, LateFusionTransformer)):
...
#
def run_cross_validation(X_eeg, X_audio, y, model_type, n_folds=5, batch_size=32, epochs=50, lr=0.001, freeze=False):
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(X_eeg, y)):
        ...
        model = create_model(model_type, num_classes=2, freeze=freeze).to(device)
        ...
        for epoch in range(1, epochs + 1):
            ...
            val_loss, val_acc, val_f1, val_probs, val_preds, val_labels = evaluate_with_probs(model, val_loader,
                                                                                              criterion, device)
